...
function writeTriple(slug, mod) {
  ...
  for (const lang of ["ru", "en", "es"]) {
    ...
    fs.writeFileSync(path.join(OUT, `${slug}.${lang}.html`), `${html.trim()}\n`, "utf8");
  }
}

writeTriple("home", home);
for (const [slug, mod] of Object.entries(corePages)) writeTriple(slug, mod);
for (const [slug, mod] of Object.entries(blogPages)) writeTriple(slug, mod);
for (const [slug, mod] of Object.entries(paymentBodies)) writeTriple(slug, mod);
for (const [slug, mod] of Object.entries(legalPages)) writeTriple(slug, mod);

console.log("Wrote content/html/*.html without artificial SEO text padding.");
